...
object Consumer5Main extends App :
  consumerBySpecifiedPartition()

  // Consume by specified partition
  def consumerBySpecifiedPartition() =
    val c = consumerClient("consumer2.properties") // Manually submit offset configuration
    val topics = List("t1-topic", "t2-topic")

    // The partition number is specified here
    val partitions = List(TopicPartition(topics.head, 0), TopicPartition(topics.head, 1))
    c.assign(partitions.asJava) // Subscribe to a specified partition of a topic
    loopForever(c)
