...
object StreamPipelineFlow1Main extends App :
  ...
  def foreachStream(builder: StreamsBuilder, topic: String) =
    ...
    source.flatMapValues(v => v.toLowerCase.split("\\s+").toList.asJava).foreach((k, v) => println(s"$k -> $v"))

  // some stream function of demo
  def wordCountStream(builder: StreamsBuilder, topicIn: String, topicOut: String) =
    val source = builder.stream[String, String](topicIn)
    val count = source
      .flatMapValues(v => v.replaceAll("[.?,:;]", " ").toLowerCase.split("\\s+").toList.asJavaCollection)
