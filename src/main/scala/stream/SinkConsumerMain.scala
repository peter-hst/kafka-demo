...
object SinkConsumerMain extends App :
  consumerWithBizConfirmCommitOffsetByPartition()

  // Manually submit offset by partition
  def consumerWithBizConfirmCommitOffsetByPartition() =

    val c = KafkaConsumer[String, Long](loadPropFile("sink1.properties")) // Manually submit offset configuration
    val topics = List("t2-topic")
    //  c.subscribe("t\\d-topic".r.pattern)
    c.subscribe(topics.asJava)
    loopForever(c)

    @tailrec
    def loopForever(c: KafkaConsumer[String, Long]): Unit =
